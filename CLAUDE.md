...
npm run verify:helm  # renders every scenario against the real chart (needs helm)
npm run parse        # regenerate src/schema.json + src/chartMeta.json
npm run fixtures     # write a values.yaml per scenario to tmp/fixtures/
```
...
```
public/values.yaml  →  parseValues.js  →  schema.json + chartMeta.json
                                              ↓
                                       displayConfig.js   (what the user sees)
                                              ↓
...
1. **`field.path` must exist in the chart.** A typo writes a key the chart ignores
   and fails silently at deploy time. `npm run validate` is the only guard — it
   evaluates `displayConfig.js` and `transform.js` output rather than pattern-matching,
   so runtime-built paths (the `secretFields` helper) are covered.
...
- `src/schema.json` and `src/chartMeta.json` are generated. Never hand-edit;
  regenerate with `npm run parse`. CI fails if they are stale.
- Port coercion keys off `field.id.includes("port")` — a port field named otherwise
  emits a quoted string the chart rejects.
- Renovate ignores `public/values.yaml` deliberately; it is a vendored copy of one
  chart release. Chart upgrades follow the procedure in `docs/architecture.md`.
